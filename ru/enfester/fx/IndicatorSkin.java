 ...
 * <i>
 * .progress-indicator {
 * 	-fx-skin: "cuina.metro.skin.IndicatorSkin";
 ...
 * </pre>
 */
public class IndicatorSkin extends ProgressIndicatorSkin // extends
		...
		// BehaviorBase<ProgressIndicator>>
{
	...
	private class MetroIndetermineSpinner extends Region
	{
		private final double timePerAngle(double x)
		{
			...
			//									 * IndicatorSkin.this.maxStepTime.get()));
		...
		private MetroIndetermineSpinner(boolean spinEnabled, Paint fillOverride)
		{
			...
			//		IndicatorSkin.this.parentTreeVisibleChangedListener);
		...
		public void setFillOverride(Paint fillOverride)
		{
			...
			this.rebuild();
			this.rebuildTimeline();
			if (IndicatorSkin.this.indeterminateTransition != null)
			{
				IndicatorSkin.this.indeterminateTransition.playFromStart();
			}
		}
		...
		private void rebuildTimeline()
		{
			if (this.spinEnabled)
			{
				if (IndicatorSkin.this.indeterminateTransition == null)
				{
					IndicatorSkin.this.indeterminateTransition = new Timeline();
					IndicatorSkin.this.indeterminateTransition
							.setCycleCount(Animation.INDEFINITE);
					IndicatorSkin.this.indeterminateTransition
							.setDelay(IndicatorSkin.this.UNCLIPPED_DELAY);
				}
				else
				{
					IndicatorSkin.this.indeterminateTransition.stop();
					((Timeline) IndicatorSkin.this.indeterminateTransition)
							.getKeyFrames().clear();
				}
				final ObservableList<KeyFrame> keyFrames = FXCollections
						.<KeyFrame>observableArrayList();

				for (int c = 0; c < IndicatorSkin.this.indeterminateSegmentCount.get(); c++)
				{
					...
					double time = IndicatorSkin.this.nextSegmentTime.get() * c;
					for (int i = 0; i <= 720; i += IndicatorSkin.this.anglePerStep
							.get())
					{
			...
			else
			{
				if (IndicatorSkin.this.indeterminateTransition != null)
				{
					...
					((Timeline) IndicatorSkin.this.indeterminateTransition)
							.getKeyFrames().clear();
					IndicatorSkin.this.indeterminateTransition = null;
				}
			}
		}
		...
		protected void layoutChildren()
		{
			final double w = IndicatorSkin.this.control.getWidth()
					- IndicatorSkin.this.control.snappedLeftInset()
					- IndicatorSkin.this.control.snappedRightInset();
			final double h = IndicatorSkin.this.control.getHeight()
					- IndicatorSkin.this.control.snappedTopInset()
					- IndicatorSkin.this.control.snappedBottomInset();
		...
		private void rebuild()
		{
			...
			final int segments = IndicatorSkin.this.indeterminateSegmentCount.get();
	...
	public IndicatorSkin(ProgressIndicator control)
	{
		super(control);
	...
	private ObjectProperty<Paint> progressColor = new StyleableObjectProperty<Paint>(null)
	{
		...
		protected void invalidated()
		{
			...
			if (IndicatorSkin.this.spinner != null)
			{
				IndicatorSkin.this.spinner.setFillOverride(value);
			}
		}
		...
		{
			return IndicatorSkin.PROGRESS_COLOR;
		}
	};
	...
	private IntegerProperty indeterminateSegmentCount = new StyleableIntegerProperty(5)
	{
		...
		protected void invalidated()
		{
			if (IndicatorSkin.this.spinner != null)
			{
				IndicatorSkin.this.spinner.rebuild();
				IndicatorSkin.this.spinner.rebuildTimeline();
				if (IndicatorSkin.this.indeterminateTransition != null)
				{
					IndicatorSkin.this.indeterminateTransition.playFromStart();
				}
		...
		{
			return IndicatorSkin.INDETERMINATE_SEGMENT_COUNT;
		}
	};
	...
	private final BooleanProperty spinEnabled = new StyleableBooleanProperty(false)
	{
		...
		protected void invalidated()
		{
			if (IndicatorSkin.this.spinner != null)
			{
				IndicatorSkin.this.spinner.setSpinEnabled(this.get());
			}
		}
		...
		{
			return IndicatorSkin.SPIN_ENABLED;
		}
	...
	private final IntegerProperty anglePerStep = new StyleableIntegerProperty(20)
	{
		...
		{
			return IndicatorSkin.ANGLE_PER_STEP;
		}
	};
	...
	private final DoubleProperty maxStepTime = new StyleableDoubleProperty(150.0d)
	{
		...
		public CssMetaData<? extends Styleable, Number> getCssMetaData()
		{
			return IndicatorSkin.MAX_STEP_TIME;
		}
	};
	...
	private final DoubleProperty nextSegmentTime = new StyleableDoubleProperty(175.0d)
	{
		...
		public CssMetaData<? extends Styleable, Number> getCssMetaData()
		{
			return IndicatorSkin.NEXT_SEGMENT_TIME;
		}
	};
	...
	private final DoubleProperty nextSpinTime = new StyleableDoubleProperty(400.0d)
	{
		...
		public CssMetaData<? extends Styleable, Number> getCssMetaData()
		{
			return IndicatorSkin.NEXT_SPIN_TIME;
		}
	};
			...
			"-fx-progress-color", PaintConverter.getInstance(), null)
	{
		...
		{
			final IndicatorSkin skin = (IndicatorSkin) n.getSkin();
			...
			"-fx-spin-enabled", BooleanConverter.getInstance(), Boolean.FALSE)
	{
		...
		{
			final IndicatorSkin skin = (IndicatorSkin) node.getSkin();
	...
	static
	{
		...
		styleables.add(IndicatorSkin.PROGRESS_COLOR);
		styleables.add(IndicatorSkin.INDETERMINATE_SEGMENT_COUNT);
		styleables.add(IndicatorSkin.SPIN_ENABLED);
		styleables.add(IndicatorSkin.ANGLE_PER_STEP);
		styleables.add(IndicatorSkin.MAX_STEP_TIME);
		styleables.add(IndicatorSkin.NEXT_SEGMENT_TIME);
		styleables.add(IndicatorSkin.NEXT_SPIN_TIME);
	...
	{
		return IndicatorSkin.STYLEABLES;
	}
	...
	{
		return IndicatorSkin.getClassCssMetaData();
	}
